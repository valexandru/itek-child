...
function itek_customize_register( $wp_customize ) {
   ...
   $wp_customize->add_section( 'itek_theme_notes' , array(
	...
	// Callout section.
	$wp_customize->add_setting(
        'itek_callout_visibility', array (
			'sanitize_callback' => 'itek_sanitize_checkbox',
    ));

    $wp_customize->add_control(
        'itek_callout_visibility',
    array(
        'type' => 'checkbox',
        'label' => __('Enable Featured Content Section?', 'itek'),
        'section' => 'callout_content',
		'priority' => 1,
        )
    );
		
	$wp_customize->add_setting( 'callout_title', array( 
	    'default' =>  __('This is a callout title', 'itek'),
        'sanitize_callback' => 'sanitize_text_field'		
	) );
	
	$wp_customize->add_control( 'callout_title', array(
        'label' => __( 'Callout Section Title', 'itek'),
        'section' => 'callout_content',
		'priority' => 2,
        'settings' => 'callout_title',
    ) );
	
	$wp_customize->add_setting( 'callout_message', array( 
	    'default' =>  __('Here goes some text to hint at what is being called out on! Keep it short, sweet & simple and an attention grabber!', 'itek'),
        'sanitize_callback' => 'sanitize_text_field'		
	) );
	
	$wp_customize->add_control( 'callout_message', array(
        'label' => __( 'Callout Message', 'itek'),
        'section' => 'callout_content',
		'priority' => 3,
        'settings' => 'callout_message',
    ) );

    $wp_customize->add_setting( 'callout_button_text', array( 
	    'default' =>  __('Click For Details!', 'itek'),
        'sanitize_callback' => 'sanitize_text_field'		
	) );
	
	$wp_customize->add_control( 'callout_button_text', array(
        'label' => __( 'Callout Button Text', 'itek'),
        'section' => 'callout_content',
		'priority' => 4,
        'settings' => 'callout_button_text',
    ) );

    $wp_customize->add_setting( 'callout_button_url', array( 
	    'default' => '#',
        'sanitize_callback' => 'esc_raw_url'		
	) );
	
	$wp_customize->add_control( 'callout_button_url', array(
        'label' => __( 'Callout Button url', 'itek'),
        'section' => 'callout_content',
		'priority' => 5,
        'settings' => 'callout_button_url',
    ) );
